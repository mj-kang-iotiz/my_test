...
void gps_port_start(gps_id_t id) {
  if (id >= GPS_ID_MAX) return;

  const board_config_t* config = board_get_config();

  // 보드와 ID에 따라 적절한 UART와 DMA 선택
  ...
  if (config->board == BOARD_TYPE_BASE_UM982 ||
    ...
    dma_stream_num = LL_DMA_STREAM_5;
             ...
             (config->board == BOARD_TYPE_ROVER_F9P && id == GPS_ID_ROVER)) {
    uart = UART4;
  ...
  // DMA 설정
  LL_DMA_SetPeriphAddress(DMA1, dma_stream_num, (uint32_t)&uart->DR);
  LL_DMA_SetMemoryAddress(DMA1, dma_stream_num, (uint32_t)gps_recv_buf[id]);
  LL_DMA_SetDataLength(DMA1, dma_stream_num, sizeof(gps_recv_buf[id]));

  LL_DMA_EnableIT_TE(DMA1, dma_stream_num);
  LL_DMA_EnableIT_FE(DMA1, dma_stream_num);
  LL_DMA_EnableIT_DME(DMA1, dma_stream_num);
  ...
  LL_DMA_EnableStream(DMA1, dma_stream_num);
  LL_USART_Enable(uart);

  // GPIO 시작 (RST 핀)
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_5, GPIO_PIN_SET);

  LOG_INFO("GPS[%d] 통신 시작 완료", id);
}
